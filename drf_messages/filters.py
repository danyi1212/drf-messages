...
class MessageFilterSet(FilterSet):
    unread = BooleanFilter(field_name="seen_at", lookup_expr="isnull", label="unread")
    ...
    seen = DateTimeFromToRangeFilter(field_name="seen_at", label="seen")
    created = DateTimeFromToRangeFilter()

    class Meta:
        model = Message
        fields = ("unread", "level_tag", "level", "extra_tags", "view", "seen", "created")
